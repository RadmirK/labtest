...
def del_patient(patient_id):
    ...
    if patient:
        db.execute('DELETE FROM PATIENTS WHERE id = %d' % patient_id)
        db.execute('DELETE FROM OAK_results WHERE patient_id = %d' % patient_id)
        db.commit()
...
def add_patient(edit_patient_id=0):
    ...
    if not error:
        if request.method == 'POST':
            insert = """INSERT into Patients (SNILS, fName, lName, mName, datebirth, gender) 
                        values (%r, %r, %r, %r, %r, %r)"""
            db.execute(insert % tuple(req_form))  # подставляем входные данные в запрос
        ...
        if request.method == 'PUT':
            ...
            db.execute(update % tuple(req_form))  # Выполняем запрос UPDATE table SET col1=val1, col2=val2, .. WHERE ..
